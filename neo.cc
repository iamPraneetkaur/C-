...
class ArrayElement{
    public:
    int search(int arr[],int x, int se){
        int c=0;
        for(int i=0; i<x; i++){
            if(arr[i]==se){
                c++;
...
int main(){
    ...
    cin>>srch_num;
    ArrayElement a;
    int count= a.search(ar,n,srch_num);
    if(count==0)
        cout<<"The number "<<srch_num<<" is not found in the array";
    else
        cout<<"The number "<<srch_num<<" appears "<<count<<" times";
}